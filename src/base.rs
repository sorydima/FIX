...
//!
//! The types defined here get used throughout fix.
use std::borrow::Cow;
...
impl fixAction {
    ...
    pub fn scribbles(&self) -> bool {
        matches!(self, fixAction::Send(SendAction::SubmitFromEditor))
    }
}

impl From<HomeserverAction> for fixAction {
    fn from(act: HomeserverAction) -> Self {
        fixAction::Homeserver(act)
    }
}

impl From<MessageAction> for fixAction {
    fn from(act: MessageAction) -> Self {
        fixAction::Message(act)
    }
}

impl From<RoomAction> for fixAction {
    fn from(act: RoomAction) -> Self {
        fixAction::Room(act)
    }
}

impl From<SendAction> for fixAction {
    fn from(act: SendAction) -> Self {
        fixAction::Send(act)
    }
}

impl ApplicationAction for fixAction {
    fn is_edit_sequence(&self, _: &EditContext) -> SequenceStatus {
        match self {
            fixAction::ClearUnreads => SequenceStatus::Break,
            fixAction::Homeserver(..) => SequenceStatus::Break,
            fixAction::Keys(..) => SequenceStatus::Break,
            fixAction::Message(..) => SequenceStatus::Break,
            fixAction::Room(..) => SequenceStatus::Break,
            fixAction::OpenLink(..) => SequenceStatus::Break,
            fixAction::Send(..) => SequenceStatus::Break,
            fixAction::ToggleScrollbackFocus => SequenceStatus::Break,
            fixAction::Verify(..) => SequenceStatus::Break,
            fixAction::VerifyRequest(..) => SequenceStatus::Break,
        }
    }

    fn is_last_action(&self, _: &EditContext) -> SequenceStatus {
        match self {
            fixAction::ClearUnreads => SequenceStatus::Atom,
            fixAction::Homeserver(..) => SequenceStatus::Atom,
            fixAction::Keys(..) => SequenceStatus::Atom,
            fixAction::Message(..) => SequenceStatus::Atom,
            fixAction::OpenLink(..) => SequenceStatus::Atom,
            fixAction::Room(..) => SequenceStatus::Atom,
            fixAction::Send(..) => SequenceStatus::Atom,
            fixAction::ToggleScrollbackFocus => SequenceStatus::Atom,
            fixAction::Verify(..) => SequenceStatus::Atom,
            fixAction::VerifyRequest(..) => SequenceStatus::Atom,
        }
    }

    fn is_last_selection(&self, _: &EditContext) -> SequenceStatus {
        match self {
            fixAction::ClearUnreads => SequenceStatus::Ignore,
            fixAction::Homeserver(..) => SequenceStatus::Ignore,
            fixAction::Keys(..) => SequenceStatus::Ignore,
            fixAction::Message(..) => SequenceStatus::Ignore,
            fixAction::Room(..) => SequenceStatus::Ignore,
            fixAction::OpenLink(..) => SequenceStatus::Ignore,
            fixAction::Send(..) => SequenceStatus::Ignore,
            fixAction::ToggleScrollbackFocus => SequenceStatus::Ignore,
            fixAction::Verify(..) => SequenceStatus::Ignore,
            fixAction::VerifyRequest(..) => SequenceStatus::Ignore,
        }
    }

    fn is_switchable(&self, _: &EditContext) -> bool {
        match self {
            fixAction::ClearUnreads => false,
            fixAction::Homeserver(..) => false,
            fixAction::Message(..) => false,
            fixAction::Room(..) => false,
            fixAction::Keys(..) => false,
            fixAction::Send(..) => false,
            fixAction::OpenLink(..) => false,
            fixAction::ToggleScrollbackFocus => false,
            fixAction::Verify(..) => false,
            fixAction::VerifyRequest(..) => false,
        }
    }
}
...
impl From<RoomAction> for ProgramAction {
    fn from(act: RoomAction) -> Self {
        fixAction::from(act).into()
    }
}

impl From<fixAction> for ProgramAction {
    fn from(act: fixAction) -> Self {
        Action::Application(act)
    }
}

/// Alias for program actions.
pub type ProgramAction = Action<fixInfo>;
/// Alias for program context.
pub type ProgramContext = EditContext;
/// Alias for program keybindings.
pub type Keybindings = VimMachine<TerminalKey, fixInfo>;
/// Alias for a program command.
pub type ProgramCommand = VimCommand<fixInfo>;
/// Alias for mapped program commands.
pub type ProgramCommands = VimCommandMachine<fixInfo>;
/// Alias for program store.
pub type ProgramStore = Store<fixInfo>;
/// Alias for shared program store.
pub type AsyncProgramStore = Arc<AsyncMutex<ProgramStore>>;
/// Alias for an action result.
pub type fixResult<T> = UIResult<T, fixInfo>;
...
impl From<fixError> for UIError<fixInfo> {
    fn from(err: fixError) -> Self {
        UIError::Application(err)
    }
}

impl ApplicationError for fixError {}
...
impl Display for fixId {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            fixId::Room(room_id, None) => {
                write!(f, "fix://room/{room_id}")
            },
            fixId::Room(room_id, Some(thread)) => {
                write!(f, "fix://room/{room_id}/threads/{thread}")
            },
            fixId::MemberList(room_id) => {
                write!(f, "fix://members/{room_id}")
            },
            fixId::DirectList => f.write_str("fix://dms"),
            fixId::RoomList => f.write_str("fix://rooms"),
            fixId::SpaceList => f.write_str("fix://spaces"),
            fixId::VerifyList => f.write_str("fix://verify"),
            fixId::Welcome => f.write_str("fix://welcome"),
            fixId::ChatList => f.write_str("fix://chats"),
            fixId::UnreadList => f.write_str("fix://unreads"),
        }
    }
}

impl ApplicationWindowId for fixId {}

impl Serialize for fixId {
...
impl<'de> Deserialize<'de> for fixId {
    fn deserialize<D>(deserializer: D) -> Result<Self, D::Error>
    where
        D: Deserializer<'de>,
    {
        deserializer.deserialize_str(fixIdVisitor)
    }
}

/// [serde] visitor for deserializing [fixId].
struct fixIdVisitor;

impl<'de> Visitor<'de> for fixIdVisitor {
    type Value = fixId;
        ...
        E: SerdeError,
    {
        let Ok(url) = Url::parse(value) else {
            return Err(E::custom("Invalid fix window URL"));
        };
        ...
        match url.domain() {
            Some("room") => {
                ...
                match *path.collect::<Vec<_>>().as_slice() {
                    ...
                    [room_id, "threads", thread_root] => {
                        ...
                        Ok(fixId::Room(room_id, Some(thread_root)))
                    },
            ...
            Some("members") => {
                ...
                Ok(fixId::MemberList(room_id))
            },
            Some("dms") => {
                if url.path() != "" {
                    return Err(E::custom("fix://dms takes no path"));
                }
            ...
            Some("rooms") => {
                if url.path() != "" {
                    return Err(E::custom("fix://rooms takes no path"));
                }
            ...
            Some("spaces") => {
                if url.path() != "" {
                    return Err(E::custom("fix://spaces takes no path"));
                }
            ...
            Some("verify") => {
                if url.path() != "" {
                    return Err(E::custom("fix://verify takes no path"));
                }
            ...
            Some("welcome") => {
                if url.path() != "" {
                    return Err(E::custom("fix://welcome takes no path"));
                }
            ...
            Some("chats") => {
                if url.path() != "" {
                    return Err(E::custom("fix://chats takes no path"));
                }
            ...
            Some("unreads") => {
                if url.path() != "" {
                    return Err(E::custom("fix://unreads takes no path"));
                }

                Ok(fixId::UnreadList)
            },
            Some(s) => Err(E::custom(format!("{s:?} is not a valid window"))),
            None => Err(E::custom("Invalid fix window URL")),
        }
    }
}
...
impl fixBufferId {
    ...
    pub fn to_window(&self) -> Option<fixId> {
        let id = match self {
            fixBufferId::Command(_) => return None,
            fixBufferId::Room(room, thread, _) => fixId::Room(room.clone(), thread.clone()),
            fixBufferId::DirectList => fixId::DirectList,
            fixBufferId::MemberList(room) => fixId::MemberList(room.clone()),
            fixBufferId::RoomList => fixId::RoomList,
            fixBufferId::SpaceList => fixId::SpaceList,
            fixBufferId::VerifyList => fixId::VerifyList,
            fixBufferId::Welcome => fixId::Welcome,
            fixBufferId::ChatList => fixId::ChatList,
            fixBufferId::UnreadList => fixId::UnreadList,
        };
...
impl ApplicationContentId for fixBufferId {}

impl ApplicationInfo for fixInfo {
    type Error = fixError;
    type Store = ChatStore;
    type Action = fixAction;
    type WindowId = fixId;
    type ContentId = fixBufferId;
    ...
    fn complete(
        ...
        match content {
            fixBufferId::Command(CommandType::Command) => complete_cmdbar(text, cursor, store),
            fixBufferId::Command(CommandType::Search) => vec![],
            fixBufferId::Room(_, _, RoomFocus::MessageBar) => complete_msgbar(text, cursor, store),
            fixBufferId::Room(_, _, RoomFocus::Scrollback) => vec![],

            fixBufferId::DirectList => vec![],
            fixBufferId::MemberList(_) => vec![],
            fixBufferId::RoomList => vec![],
            fixBufferId::SpaceList => vec![],
            fixBufferId::VerifyList => vec![],
            fixBufferId::Welcome => vec![],
            fixBufferId::ChatList => vec![],
            fixBufferId::UnreadList => vec![],
        }
    }

    fn content_of_command(ct: CommandType) -> fixBufferId {
        fixBufferId::Command(ct)
    }
}
