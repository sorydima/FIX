...
const fix_DEVICE_NAME: &str = "fix";
const fix_USER_AGENT: &str = "fix";
const MIN_MSG_LOAD: u32 = 50;

type MessageFetchResult =
    fixResult<(Option<String>, Vec<(AnyMessageLikeEvent, Vec<OwnedUserId>)>)>;

fn initial_devname() -> String {
    format!("{} on {}", fix_DEVICE_NAME, gethostname().to_string_lossy())
}
...
pub async fn create_room(
    ...
    match rt {
        ...
        CreateRoomType::Space => {
            ...
            let raw_cc = Raw::new(&cc).map_err(fixError::from)?;
    ...
    if flags.contains(CreateRoomFlags::ENCRYPTED) {
        ...
        let encr_raw = Raw::new(&encr).map_err(fixError::from)?;
    ...
    let resp = client.create_room(request).await.map_err(fixError::from)?;

    if is_direct {
        if let Some(room) = client.get_room(resp.room_id()) {
            room.set_is_direct(true).await.map_err(fixError::from)?;
        } else {
            error!(
...
async fn load_older_one(
    ...
    if let Some(room) = client.get_room(room_id) {
        ...
        let Messages { end, chunk, .. } = room.messages(opts).await.map_err(fixError::from)?;
        ...
        Err(fixError::UnknownRoom(room_id.to_owned()).into())
    }
}
...
async fn members_load(client: &Client, room_id: &RoomId) -> fixResult<Vec<RoomMember>> {
    if let Some(room) = client.get_room(room_id) {
        Ok(room
...
pub enum WorkerTask {
    ...
    Login(LoginStyle, ClientReply<fixResult<EditInfo>>),
    Logout(String, ClientReply<fixResult<EditInfo>>),
    GetInviter(MatrixRoom, ClientReply<fixResult<Option<RoomMember>>>),
    GetRoom(OwnedRoomId, ClientReply<fixResult<FetchedRoom>>),
    JoinRoom(String, ClientReply<fixResult<OwnedRoomId>>),
    Members(OwnedRoomId, ClientReply<fixResult<Vec<RoomMember>>>),
    SpaceMembers(OwnedRoomId, ClientReply<fixResult<Vec<OwnedRoomId>>>),
    TypingNotice(OwnedRoomId),
    Verify(VerifyAction, SasVerification, ClientReply<fixResult<EditInfo>>),
    VerifyRequest(OwnedUserId, ClientReply<fixResult<EditInfo>>),
}
...
impl Requester {
    ...
    pub fn login(&self, style: LoginStyle) -> fixResult<EditInfo> {
        let (reply, response) = oneshot();
    ...
    pub fn logout(&self, user_id: String) -> fixResult<EditInfo> {
        let (reply, response) = oneshot();
    ...
    pub fn get_inviter(&self, invite: MatrixRoom) -> fixResult<Option<RoomMember>> {
        let (reply, response) = oneshot();
    ...
    pub fn get_room(&self, room_id: OwnedRoomId) -> fixResult<FetchedRoom> {
        let (reply, response) = oneshot();
    ...
    pub fn join_room(&self, name: String) -> fixResult<OwnedRoomId> {
        let (reply, response) = oneshot();
    ...
    pub fn members(&self, room_id: OwnedRoomId) -> fixResult<Vec<RoomMember>> {
        let (reply, response) = oneshot();
    ...
    pub fn space_members(&self, space: OwnedRoomId) -> fixResult<Vec<OwnedRoomId>> {
        let (reply, response) = oneshot();
    ...
    pub fn verify(&self, act: VerifyAction, sas: SasVerification) -> fixResult<EditInfo> {
        let (reply, response) = oneshot();
    ...
    pub fn verify_request(&self, user_id: OwnedUserId) -> fixResult<EditInfo> {
        let (reply, response) = oneshot();
...
impl ClientWorker {
    ...
    async fn login_and_sync(&mut self, style: LoginStyle) -> fixResult<EditInfo> {
        let client = self.client.clone();

        match style {
            LoginStyle::SessionRestore(session) => {
                client.restore_session(session).await.map_err(fixError::from)?;
            },
    ...
    async fn logout(&mut self, user_id: String) -> fixResult<EditInfo> {
    ...
    async fn direct_message(&mut self, user: OwnedUserId) -> fixResult<OwnedRoomId> {
        for room in self.client.rooms() {
            if !is_direct(&room).await {
                continue;
            }

            if room.get_member(user.as_ref()).await.map_err(fixError::from)?.is_some() {
    ...
    async fn get_inviter(&mut self, invited: MatrixRoom) -> fixResult<Option<RoomMember>> {
        let details = invited.invite_details().await.map_err(fixError::from)?;

        Ok(details.inviter)
    }

    async fn get_room(&mut self, room_id: OwnedRoomId) -> fixResult<FetchedRoom> {
        if let Some(room) = self.client.get_room(&room_id) {
            let name = room.display_name().await.map_err(fixError::from)?;
            let tags = room.tags().await.map_err(fixError::from)?;

            Ok((room, name, tags))
        } else {
            Err(fixError::UnknownRoom(room_id).into())
        }
    }

    async fn join_room(&mut self, name: String) -> fixResult<OwnedRoomId> {
    ...
    async fn members(&mut self, room_id: OwnedRoomId) -> fixResult<Vec<RoomMember>> {
        if let Some(room) = self.client.get_room(room_id.as_ref()) {
            Ok(room.members(RoomMemberships::ACTIVE).await.map_err(fixError::from)?)
        } else {
            Err(fixError::UnknownRoom(room_id).into())
        }
    }

    async fn space_members(&mut self, space: OwnedRoomId) -> fixResult<Vec<OwnedRoomId>> {
        let mut req = SpaceHierarchyRequest::new(space);
        req.limit = Some(1000u32.into());
        req.max_depth = Some(1u32.into());

        let resp = self.client.send(req, None).await.map_err(fixError::from)?;
    ...
    async fn verify(&self, action: VerifyAction, sas: SasVerification) -> fixResult<EditInfo> {
        match action {
            VerifyAction::Accept => {
                sas.accept().await.map_err(fixError::from)?;
            ...
            VerifyAction::Confirm => {
                ...
                sas.confirm().await.map_err(fixError::from)?;
            ...
            VerifyAction::Cancel => {
                ...
                sas.cancel().await.map_err(fixError::from)?;
            ...
            VerifyAction::Mismatch => {
                ...
                sas.mismatch().await.map_err(fixError::from)?;
    ...
    async fn verify_request(&self, user_id: OwnedUserId) -> fixResult<EditInfo> {
        let enc = self.client.encryption();

        match enc.get_user_identity(user_id.as_ref()).await.map_err(fixError::from)? {
            Some(identity) => {
                let methods = vec![VerificationMethod::SasV1];
                let request = identity.request_verification_with_methods(methods);
                let _req = request.await.map_err(fixError::from)?;
