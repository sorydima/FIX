//! # Default Keybindings
//!
//! The keybindings are set up here. We define some fix-specific keybindings, but the default Vim
...
use crate::base::{fixAction, fixInfo, Keybindings, MATRIX_ID_WORD};
use crate::config::{ApplicationSettings, Keys};

pub type fixStep = InputStep<fixInfo>;
...
pub fn setup_keybindings() -> Keybindings {
    ...
    let stoggle = fixStep::new()
        .actions(vec![fixAction::ToggleScrollbackFocus.into()])
        .goto(VimMode::Normal);
    ...
    let newline = fixStep::new().actions(vec![InsertTextAction::Type(
        Char::Single('\n').into(),
        MoveDir1D::Previous,
        1.into(),
    )
...
impl InputBindings<TerminalKey, fixStep> for ApplicationSettings {
    fn setup(&self, bindings: &mut Keybindings) {
        for (modes, keys) in &self.macros {
            for (Keys(input, _), Keys(_, run)) in keys {
                ...
                let step = fixStep::new().actions(vec![act.into()]);
