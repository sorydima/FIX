...
use crate::{
    base::{AsyncProgramStore, ChatStore, fixError},
...
pub fn spawn_insert_preview(
    ...
    tokio::spawn(async move {
        let img = download_or_load(event_id.to_owned(), source, media, cache_dir)
            ...
            .map_err(fixError::Matrix)
            .and_then(|reader| reader.with_guessed_format().map_err(fixError::IOError))
            .and_then(|reader| reader.decode().map_err(fixError::Image));
        ...
        match img {
            ...
            Ok(img) => {
                ...
                match picker
                    .as_mut()
                    .ok_or_else(|| fixError::Preview("Picker is empty".to_string()))
                    .and_then(|picker| {
                        Ok((
                            picker,
                            rooms
                                ...
                                .ok_or_else(|| {
                                    fixError::Preview("Message not found".to_string())
                                })?,
                            settings.tunables.image_preview.clone().ok_or_else(|| {
                                fixError::Preview("image_preview settings not found".to_string())
                            })?,
                        ))
                    })
                    .and_then(|(picker, msg, image_preview)| {
                        picker
                            ...
                            .map_err(|err| fixError::Preview(format!("{err:?}")))
...
fn try_set_msg_preview_error(
    ...
    match rooms
        .get_or_default(room_id.clone())
        .get_event_mut(&event_id)
        .ok_or_else(|| fixError::Preview("Message not found".to_string()))
    {
