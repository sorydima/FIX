//! # fix
//!
//! The fix client loops over user input and commands, and turns them into actions, [some of
//! which][fixAction] are specific to fix, and [some of which][Action] come from [modalkit]. When
//! adding new functionality, you will usually want to extend [fixAction] or one of its variants
...
use crate::{
    ...
    config::{ApplicationSettings, fix},
    windows::fixWindow,
...
) -> fixResult<WindowLayoutDescription<fixInfo>> {
    let desc = match layout {
        config::WindowLayout::Window { window } => {
            ...
            let window = match window {
                ...
                config::WindowPath::RoomId(room_id) => fixId::Room(room_id, None),
...
fn restore_layout(
    ...
    store: &mut ProgramStore,
) -> fixResult<FocusList<WindowLayoutState<fixWindow, fixInfo>>> {
    let layout = std::fs::read(&settings.layout_json)?;
    let tabs: TabbedLayoutDescription<fixInfo> =
        serde_json::from_slice(&layout).map_err(fixError::from)?;
...
fn setup_screen(
    settings: ApplicationSettings,
    store: &mut ProgramStore,
) -> fixResult<ScreenState<fixWindow, fixInfo>> {
    ...
    let win = settings
        .tunables
        .default_room
        .and_then(|room| fixWindow::find(room, store).ok())
        .or_else(|| fixWindow::open(fixId::Welcome, store).ok())
        .unwrap();
...
struct Application {
    ...
    screen: ScreenState<fixWindow, fixInfo>,
    ...
    last_layout: Option<TabbedLayoutDescription<fixInfo>>,
...
impl Application {
    ...
    async fn action_run(
        ...
        let info = match action {
            ...
            // Simple delegations.
            Action::Application(act) => self.fix_run(act, ctx, store).await?,
    ...
    async fn fix_run(
        ...
        let info = match action {
            ...
            fixAction::ToggleScrollbackFocus => {
            ...
            fixAction::Keys(act) => self.keys_command(act, ctx, store).await?,
    ...
    async fn homeserver_command(
        ...
        match action {
            HomeserverAction::CreateRoom(alias, vis, flags) => {
                ...
                let room = fixId::Room(room_id, None);
            ...
            HomeserverAction::Logout(user, false) => {
                ...
                let act = fixAction::from(HomeserverAction::Logout(user, true));
...
async fn login(worker: &Requester, settings: &ApplicationSettings) -> fixResult<()> {
...
async fn check_import_keys(
    settings: &ApplicationSettings,
) -> fixResult<Option<(temp_dir::TempDir, String)>> {
...
async fn run(settings: ApplicationSettings) -> fixResult<()> {
    ...
    match res {
        Err(UIError::Application(fixError::Matrix(e))) => {
...
fn main() -> fixResult<()> {
    ...
    let settings = ApplicationSettings::load(fix).unwrap_or_else(print_exit);
    ...
    let log_prefix = format!("fix-log-{}", settings.profile_name);
