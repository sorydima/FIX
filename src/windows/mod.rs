...
//!
//! Additionally, some of the fix commands delegate behaviour to the current UI element. For
...
fn room_prompt(
    ...
    act: &PromptAction,
    ctx: &ProgramContext,
) -> EditResult<Vec<(ProgramAction, ProgramContext)>, fixInfo> {
    match act {
        PromptAction::Submit => {
            let room = fixId::Room(room_id.to_owned(), None);
...
macro_rules! delegate {
    ($s: expr, $id: ident => $e: expr) => {
        match $s {
            fixWindow::Room($id) => $e,
            fixWindow::DirectList($id) => $e,
            fixWindow::MemberList($id, _, _) => $e,
            fixWindow::RoomList($id) => $e,
            fixWindow::SpaceList($id) => $e,
            fixWindow::VerifyList($id) => $e,
            fixWindow::Welcome($id) => $e,
            fixWindow::ChatList($id) => $e,
            fixWindow::UnreadList($id) => $e,
        }
    };
}
...
impl fixWindow {
    ...
    pub async fn message_command(
        ...
        if let fixWindow::Room(w) = self {
            ...
            return Err(fixError::NoSelectedRoom.into());
        }
    }
    ...
    pub async fn room_command(
        ...
        ctx: ProgramContext,
        store: &mut ProgramStore,
    ) -> fixResult<Vec<(Action<fixInfo>, ProgramContext)>> {
        if let fixWindow::Room(w) = self {
            w.room_command(act, ctx, store).await
        } else {
            return Err(fixError::NoSelectedRoomOrSpace.into());
        }
    }
...
pub type DirectListState = ListState<DirectItem, fixInfo>;
pub type MemberListState = ListState<MemberItem, fixInfo>;
pub type RoomListState = ListState<RoomItem, fixInfo>;
pub type ChatListState = ListState<GenericChatItem, fixInfo>;
pub type UnreadListState = ListState<GenericChatItem, fixInfo>;
pub type SpaceListState = ListState<SpaceItem, fixInfo>;
pub type VerifyListState = ListState<VerifyItem, fixInfo>;

impl From<ChatListState> for fixWindow {
    fn from(list: ChatListState) -> Self {
        fixWindow::ChatList(list)
    }
}

impl From<RoomState> for fixWindow {
    fn from(room: RoomState) -> Self {
        fixWindow::Room(room)
    }
}

impl From<VerifyListState> for fixWindow {
    fn from(list: VerifyListState) -> Self {
        fixWindow::VerifyList(list)
    }
}

impl From<DirectListState> for fixWindow {
    fn from(list: DirectListState) -> Self {
        fixWindow::DirectList(list)
    }
}

impl From<RoomListState> for fixWindow {
    fn from(list: RoomListState) -> Self {
        fixWindow::RoomList(list)
    }
}

impl From<SpaceListState> for fixWindow {
    fn from(list: SpaceListState) -> Self {
        fixWindow::SpaceList(list)
    }
}

impl From<WelcomeState> for fixWindow {
    fn from(win: WelcomeState) -> Self {
        fixWindow::Welcome(win)
    }
}

impl Editable<ProgramContext, ProgramStore, fixInfo> for fixWindow {
    fn editor_command(
        &mut self,
        act: &EditorAction,
        ctx: &ProgramContext,
        store: &mut ProgramStore,
    ) -> EditResult<EditInfo, fixInfo> {
        delegate!(self, w => w.editor_command(act, ctx, store))
    }
}

impl Jumpable<ProgramContext, fixInfo> for fixWindow {
    fn jump(
        &mut self,
        list: PositionList,
        dir: MoveDir1D,
...
impl Scrollable<ProgramContext, ProgramStore, fixInfo> for fixWindow {
    fn scroll(
        &mut self,
        style: &ScrollStyle,
        ctx: &ProgramContext,
...
impl Promptable<ProgramContext, ProgramStore, fixInfo> for fixWindow {
    fn prompt(
        &mut self,
        act: &PromptAction,
        ctx: &ProgramContext,
...
impl TerminalCursor for fixWindow {
    fn get_term_cursor(&self) -> Option<TermOffset> {
        delegate!(self, w => w.get_term_cursor())
    }
}

impl WindowOps<fixInfo> for fixWindow {
    fn draw(&mut self, area: Rect, buf: &mut Buffer, focused: bool, store: &mut ProgramStore) {
        match self {
            fixWindow::Room(state) => state.draw(area, buf, focused, store),
            fixWindow::DirectList(state) => {
                let mut items = store
                    .application
                    .sync_info
                    .dms
            ...
            fixWindow::MemberList(state, room_id, last_fetch) => {
            ...
            fixWindow::Welcome(state) => state.draw(area, buf, focused, store),
        }
    }

    fn dup(&self, store: &mut ProgramStore) -> Self {
        match self {
            fixWindow::Room(w) => w.dup(store).into(),
            fixWindow::DirectList(w) => w.dup(store).into(),
            fixWindow::MemberList(w, room_id, last_fetch) => {
                fixWindow::MemberList(w.dup(store), room_id.clone(), *last_fetch)
            },
            fixWindow::RoomList(w) => w.dup(store).into(),
            fixWindow::SpaceList(w) => w.dup(store).into(),
            fixWindow::VerifyList(w) => w.dup(store).into(),
            fixWindow::Welcome(w) => w.dup(store).into(),
            fixWindow::ChatList(w) => w.dup(store).into(),
            fixWindow::UnreadList(w) => w.dup(store).into(),
        }
    }
...
impl Window<fixInfo> for fixWindow {
    fn id(&self) -> fixId {
        match self {
            fixWindow::Room(room) => fixId::Room(room.id().to_owned(), room.thread().cloned()),
            fixWindow::DirectList(_) => fixId::DirectList,
            fixWindow::MemberList(_, room_id, _) => fixId::MemberList(room_id.clone()),
            fixWindow::RoomList(_) => fixId::RoomList,
            fixWindow::SpaceList(_) => fixId::SpaceList,
            fixWindow::VerifyList(_) => fixId::VerifyList,
            fixWindow::Welcome(_) => fixId::Welcome,
            fixWindow::ChatList(_) => fixId::ChatList,
            fixWindow::UnreadList(_) => fixId::UnreadList,
        }
    }

    fn get_tab_title(&self, store: &mut ProgramStore) -> Line {
        match self {
            fixWindow::DirectList(_) => bold_spans("Direct Messages"),
            fixWindow::RoomList(_) => bold_spans("Rooms"),
            fixWindow::SpaceList(_) => bold_spans("Spaces"),
            fixWindow::VerifyList(_) => bold_spans("Verifications"),
            fixWindow::Welcome(_) => bold_spans("Welcome to fix"),
            fixWindow::ChatList(_) => bold_spans("DMs & Rooms"),
            fixWindow::UnreadList(_) => bold_spans("Unread Messages"),
            ...
            fixWindow::MemberList(state, room_id, _) => {
    ...
    fn get_win_title(&self, store: &mut ProgramStore) -> Line {
        match self {
            ...
            fixWindow::Room(w) => w.get_title(store),
            fixWindow::MemberList(state, room_id, _) => {
                let title = store.application.get_room_title(room_id.as_ref());
                let n = state.len();
                let v = vec![
    ...
    fn open(id: fixId, store: &mut ProgramStore) -> fixResult<Self> {
        match id {
            fixId::Room(room_id, thread) => {
            ...
            fixId::DirectList => {
                let list = DirectListState::new(fixBufferId::DirectList, vec![]);

                return Ok(list.into());
            },
            fixId::MemberList(room_id) => {
                let id = fixBufferId::MemberList(room_id.clone());
                let list = MemberListState::new(id, vec![]);
                let win = fixWindow::MemberList(list, room_id, None);

                return Ok(win);
            },
            fixId::RoomList => {
                let list = RoomListState::new(fixBufferId::RoomList, vec![]);

                return Ok(list.into());
            },
            fixId::SpaceList => {
                let list = SpaceListState::new(fixBufferId::SpaceList, vec![]);

                return Ok(list.into());
            },
            fixId::VerifyList => {
                let list = VerifyListState::new(fixBufferId::VerifyList, vec![]);
            ...
            fixId::ChatList => {
                let list = ChatListState::new(fixBufferId::ChatList, vec![]);

                Ok(list.into())
            },
            fixId::UnreadList => {
                let list = UnreadListState::new(fixBufferId::UnreadList, vec![]);

                Ok(fixWindow::UnreadList(list))
            },
        }
    }

    fn find(name: String, store: &mut ProgramStore) -> fixResult<Self> {
        let ChatStore { names, worker, .. } = &mut store.application;

        if let Some(room) = names.get_mut(&name) {
            let id = fixId::Room(room.clone(), None);

            fixWindow::open(id, store)
        } else {
    ...
    fn posn(index: usize, _: &mut ProgramStore) -> fixResult<Self> {
    ...
    fn unnamed(store: &mut ProgramStore) -> fixResult<Self> {
        Self::open(fixId::RoomList, store)
    }
}
...
impl ListItem<fixInfo> for GenericChatItem {
...
impl Promptable<ProgramContext, ProgramStore, fixInfo> for GenericChatItem {
    fn prompt(
        &mut self,
        act: &PromptAction,
        ctx: &ProgramContext,
...
impl ListItem<fixInfo> for RoomItem {
    fn show(&self, selected: bool, _: &ViewportContext<ListCursor>, _: &mut ProgramStore) -> Text {
        let unread = self.unread.is_unread();
        let style = selected_style(selected);
        let (name, mut labels) = name_and_labels(&self.name, unread, style);
...
impl Promptable<ProgramContext, ProgramStore, fixInfo> for RoomItem {
    fn prompt(
        &mut self,
        act: &PromptAction,
        ctx: &ProgramContext,
...
impl ListItem<fixInfo> for DirectItem {
    fn show(&self, selected: bool, _: &ViewportContext<ListCursor>, _: &mut ProgramStore) -> Text {
        let unread = self.unread.is_unread();
        let style = selected_style(selected);
        let (name, mut labels) = name_and_labels(&self.name, unread, style);
...
impl Promptable<ProgramContext, ProgramStore, fixInfo> for DirectItem {
    fn prompt(
        &mut self,
        act: &PromptAction,
        ctx: &ProgramContext,
...
impl ListItem<fixInfo> for SpaceItem {
    fn show(&self, selected: bool, _: &ViewportContext<ListCursor>, _: &mut ProgramStore) -> Text {
...
impl Promptable<ProgramContext, ProgramStore, fixInfo> for SpaceItem {
    fn prompt(
        &mut self,
        act: &PromptAction,
        ctx: &ProgramContext,
...
impl ListItem<fixInfo> for VerifyItem {
    fn show(&self, selected: bool, _: &ViewportContext<ListCursor>, _: &mut ProgramStore) -> Text {
        let mut lines = vec![];
...
impl Promptable<ProgramContext, ProgramStore, fixInfo> for VerifyItem {
    fn prompt(
        &mut self,
        act: &PromptAction,
        _: &ProgramContext,
...
impl ListItem<fixInfo> for MemberItem {
    fn show(
        &self,
        selected: bool,
...
impl Promptable<ProgramContext, ProgramStore, fixInfo> for MemberItem {
    fn prompt(
        &mut self,
        act: &PromptAction,
        _: &ProgramContext,
