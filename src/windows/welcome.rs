...
use crate::base::{fixBufferId, fixInfo, fixResult, ProgramStore};
...
impl WelcomeState {
    pub fn new(store: &mut ProgramStore) -> Self {
        let buf = store.buffers.load_str(fixBufferId::Welcome, WELCOME_TEXT);
...
impl Deref for WelcomeState {
    type Target = TextBoxState<fixInfo>;
...
impl WindowOps<fixInfo> for WelcomeState {
