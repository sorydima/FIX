...
impl ScrollbackState {
    pub fn new(room_id: OwnedRoomId, thread: Option<OwnedEventId>) -> ScrollbackState {
        let id = fixBufferId::Room(room_id.to_owned(), thread.clone(), RoomFocus::Scrollback);
...
impl WindowOps<fixInfo> for ScrollbackState {
...
impl EditorActions<ProgramContext, ProgramStore, fixInfo> for ScrollbackState {
    fn edit(
        &mut self,
...
impl Editable<ProgramContext, ProgramStore, fixInfo> for ScrollbackState {
    fn editor_command(
        &mut self,
        act: &EditorAction,
        ctx: &ProgramContext,
...
impl Jumpable<ProgramContext, fixInfo> for ScrollbackState {
    fn jump(
        &mut self,
        list: PositionList,
        dir: MoveDir1D,
...
impl Promptable<ProgramContext, ProgramStore, fixInfo> for ScrollbackState {
    fn prompt(
        &mut self,
        act: &PromptAction,
        ctx: &ProgramContext,
        store: &mut ProgramStore,
    ) -> EditResult<Vec<(Action<fixInfo>, ProgramContext)>, fixInfo> {
        let info = store.application.get_room_info(self.room_id.clone());
        let thread = self.get_thread(info).ok_or_else(no_msgs)?;
        ...
        match act {
            PromptAction::Submit => {
                if self.thread.is_some() {
                    ...
                    let id = fixId::Room(room_id, Some(root));
...
impl ScrollActions<ProgramContext, ProgramStore, fixInfo> for ScrollbackState {
    fn dirscroll(
        &mut self,
        dir: MoveDir2D,
        size: ScrollSize,
...
impl Scrollable<ProgramContext, ProgramStore, fixInfo> for ScrollbackState {
    fn scroll(
        &mut self,
        style: &ScrollStyle,
        ctx: &ProgramContext,
...
impl Searchable<ProgramContext, ProgramStore, fixInfo> for ScrollbackState {
    fn search(
        &mut self,
        dir: MoveDirMod,
        count: Count,
