...
impl ChatState {
    pub fn new(room: MatrixRoom, thread: Option<OwnedEventId>, store: &mut ProgramStore) -> Self {
        ...
        let id = fixBufferId::Room(room_id.clone(), thread, RoomFocus::MessageBar);
    ...
    fn get_joined(&self, worker: &Requester) -> Result<MatrixRoom, fixError> {
        let Some(room) = worker.client.get_room(self.id()) else {
            return Err(fixError::NotJoined);
        };

        if room.state() == RoomState::Joined {
            Ok(room)
        } else {
            Err(fixError::NotJoined)
        }
    }
    ...
    pub async fn message_command(
        ...
        let msg = self.scrollback.get_mut(info).ok_or(fixError::NoSelectedMessage)?;
        ...
        match act {
            ...
            MessageAction::Download(filename, flags) => {
                if let MessageEvent::Original(ev) = &msg.event {
                    ...
                    let mut filename = match (filename, &settings.dirs.downloads) {
                        ...
                        (None, None) => return Err(fixError::NoDownloadDir.into()),
                    };
                    ...
                    let (source, msg_filename) = match &ev.content.msgtype {
                        ...
                        _ => {
                            if !flags.contains(DownloadFlags::OPEN) {
                                return Err(fixError::NoAttachment.into());
                            }
                            ...
                            let choices = links
                                ...
                                .map(|l| {
                                    ...
                                    let act = fixAction::OpenLink(url.clone()).into();
                    ...
                    if !filename.exists() || flags.contains(DownloadFlags::FORCE) {
                        ...
                        let bytes =
                            media.get_media_content(&req, true).await.map_err(fixError::from)?;
                ...
                Err(fixError::NoAttachment.into())
            },
            MessageAction::Edit => {
            ...
            MessageAction::React(reaction, literal) => {
                    ...
                    emojis::get(&reaction).or_else(|| emojis::get_by_shortcode(&reaction))
                {
                    ...
                    let act = fixAction::Message(MessageAction::React(reaction, true));
                    let prompt = PromptYesNo::new(msg, vec![Action::from(act)]);
                    let prompt = Box::new(prompt);
                ...
                let _ = room.send(msg).await.map_err(fixError::from)?;
            ...
            MessageAction::Redact(reason, skip_confirm) => {
                if !skip_confirm {
                    ...
                    let act = fixAction::Message(MessageAction::Redact(reason, true));
                    let prompt = PromptYesNo::new(msg, vec![Action::from(act)]);
                    let prompt = Box::new(prompt);
                ...
                let _ = room.redact(event_id, reason, None).await.map_err(fixError::from)?;
            ...
            MessageAction::Unreact(reaction, literal) => {
                let emoji = match reaction {
                    ...
                    Some(reaction) => {
                            ...
                            emojis::get(&reaction).or_else(|| emojis::get_by_shortcode(&reaction))
                        {
                            ...
                            let act =
                                fixAction::Message(MessageAction::Unreact(Some(reaction), true));
                            let prompt = PromptYesNo::new(msg, vec![Action::from(act)]);
                            let prompt = Box::new(prompt);
                ...
                for reaction in reactions {
                    let _ = room.redact(reaction, None, None).await.map_err(fixError::from)?;
                }
    ...
    pub async fn send_command(
        ...
        let (event_id, msg) = match act {
            SendAction::Submit | SendAction::SubmitFromEditor => {
                ...
                let resp = room.send(msg.clone()).await.map_err(fixError::from)?;
...
impl WindowOps<fixInfo> for ChatState {
...
impl Editable<ProgramContext, ProgramStore, fixInfo> for ChatState {
    fn editor_command(
        &mut self,
        act: &EditorAction,
        ctx: &ProgramContext,
        ...
        match delegate!(self, w => w.editor_command(act, ctx, store)) {
            res @ Ok(_) => res,
            Err(EditError::WrongBuffer(fixBufferId::Room(room_id, thread, focus)))
            ...
            Err(EditError::Register(RegisterError::ClipboardImage(data))) => {
                ...
                let send =
                    fixAction::Send(SendAction::UploadImage(data.width, data.height, data.bytes));
...
impl Jumpable<ProgramContext, fixInfo> for ChatState {
    fn jump(
        &mut self,
        list: PositionList,
        dir: MoveDir1D,
...
impl Scrollable<ProgramContext, ProgramStore, fixInfo> for ChatState {
    fn scroll(
        &mut self,
        style: &ScrollStyle,
        ctx: &ProgramContext,
...
impl PromptActions<ProgramContext, ProgramStore, fixInfo> for ChatState {
    fn submit(
        &mut self,
        ctx: &ProgramContext,
        _: &mut ProgramStore,
        ...
        Ok(vec![(fixAction::from(act).into(), ctx.clone())])
    }
...
impl Promptable<ProgramContext, ProgramStore, fixInfo> for ChatState {
    fn prompt(
        &mut self,
        act: &PromptAction,
        ctx: &ProgramContext,
...
fn open_command(open_command: Option<&Vec<String>>, target: OsString) -> fixResult<()> {
