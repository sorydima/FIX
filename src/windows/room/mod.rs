...
fn notification_mode(name: impl Into<String>) -> fixResult<RoomNotificationMode> {
    let name = name.into();
    ...
    let mode = match name.to_lowercase().as_str() {
        ...
        _ => return Err(fixError::InvalidNotificationLevel(name).into()),
    };

    Ok(mode)
}

fn hist_visibility_mode(name: impl Into<String>) -> fixResult<HistoryVisibility> {
    let name = name.into();
    ...
    let mode = match name.to_lowercase().as_str() {
        ...
        _ => return Err(fixError::InvalidHistoryVisibility(name).into()),
    };
...
impl RoomState {
    ...
    pub async fn message_command(
        ...
        match self {
            ...
            RoomState::Space(_) => Err(fixError::NoSelectedMessage.into()),
        }
    }
    ...
    pub async fn send_command(
        ...
        match self {
            ...
            RoomState::Space(_) => Err(fixError::NoSelectedRoom.into()),
        }
    }
        ...
        match act {
            RoomAction::InviteAccept => {
                if let Some(room) = store.application.worker.client.get_room(self.id()) {
                    ...
                    Err(fixError::NotInvited.into())
                }
            },
            RoomAction::InviteReject => {
                if let Some(room) = store.application.worker.client.get_room(self.id()) {
                    room.leave().await.map_err(fixError::from)?;
            ...
            RoomAction::InviteSend(user) => {
                if let Some(room) = store.application.worker.client.get_room(self.id()) {
                    room.invite_user_by_id(user.as_ref()).await.map_err(fixError::from)?;

                    Ok(vec![])
                } else {
                    Err(fixError::NotJoined.into())
                }
            },
            RoomAction::Leave(skip_confirm) => {
                if let Some(room) = store.application.worker.client.get_room(self.id()) {
                    if skip_confirm {
                        ...
                        let leave = fixAction::Room(RoomAction::Leave(true));
            ...
            RoomAction::MemberUpdate(mua, user, reason, skip_confirm) => {
                let Some(room) = store.application.worker.client.get_room(self.id()) else {
                    return Err(fixError::NotJoined.into());
                };

                let Ok(user_id) = OwnedUserId::try_from(user.as_str()) else {
                    let err = fixError::InvalidUserId(user);
            ...
            RoomAction::Members(mut cmd) => {
                ...
                let act =
                    cmd.default_axis(Axis::Vertical).default_relation(MoveDir1D::Next).window(
                        OpenTarget::Application(fixId::MemberList(self.id().to_owned())),
                        width.into(),
                    );
            ...
            RoomAction::SetDirect(is_direct) => {
                let room = store
                    ...
                    .ok_or(UIError::Application(fixError::NotJoined))?;

                room.set_is_direct(is_direct).await.map_err(fixError::from)?;
            ...
            RoomAction::Set(field, value) => {
                ...
                match field {
                    RoomField::History => {
                        ...
                        let _ = room.send_state_event(ev).await.map_err(fixError::from)?;
                    },
                    RoomField::Name => {
                    ...
                    RoomField::Tag(tag) => {
                        ...
                        let _ = room.set_tag(tag, info).await.map_err(fixError::from)?;
                    },
                    RoomField::Topic => {
                    ...
                    RoomField::CanonicalAlias => {
                        ...
                        let Ok(orai) = OwnedRoomAliasId::try_from(value.as_str()) else {
                            let err = fixError::InvalidRoomAlias(value);
                        ...
                        if let Err(e) = client.send(alias_create_req, None).await {
                            if let Some(ClientApiErrorKind::Unknown) = e.client_api_error_kind() {
                                ...
                                return Err(fixError::from(e).into());
                            }
                        }
                    ...
                    RoomField::Alias(alias) => {
                        ...
                        let Ok(orai) = OwnedRoomAliasId::try_from(alias.as_str()) else {
                            let err = fixError::InvalidRoomAlias(alias);
            ...
            RoomAction::Unset(field) => {
                ...
                match field {
                    ...
                    RoomField::Tag(tag) => {
                        let _ = room.remove_tag(tag).await.map_err(fixError::from)?;
                    },
                    RoomField::Topic => {
...
impl Editable<ProgramContext, ProgramStore, fixInfo> for RoomState {
    fn editor_command(
        &mut self,
        act: &EditorAction,
        ctx: &ProgramContext,
...
impl Jumpable<ProgramContext, fixInfo> for RoomState {
    fn jump(
        &mut self,
        list: PositionList,
        dir: MoveDir1D,
...
impl Scrollable<ProgramContext, ProgramStore, fixInfo> for RoomState {
    fn scroll(
        &mut self,
        style: &ScrollStyle,
        ctx: &ProgramContext,
...
impl Promptable<ProgramContext, ProgramStore, fixInfo> for RoomState {
    fn prompt(
        &mut self,
        act: &PromptAction,
        ctx: &ProgramContext,
...
impl WindowOps<fixInfo> for RoomState {
