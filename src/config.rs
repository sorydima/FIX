...
impl ApplicationSettings {
    pub fn load(cli: fix) -> Result<Self, Box<dyn std::error::Error>> {
        ...
        let config = if config_toml.is_file() {
            fixConfig::load_toml(config_toml.as_path())?
        } else if config_json.is_file() {
            fixConfig::load_json(config_json.as_path())?
        } else {
            usage!(
    ...
    pub fn read_session(&self, path: impl AsRef<Path>) -> Result<Session, fixError> {
        let file = File::open(path)?;
        let reader = BufReader::new(file);
        let session = serde_json::from_reader(reader).map_err(fixError::from)?;
        Ok(session)
    }

    pub fn write_session(&self, session: MatrixSession) -> Result<(), fixError> {
        let file = File::create(self.session_json.as_path())?;
        let writer = BufWriter::new(file);
        let session = Session::from(session);
        serde_json::to_writer(writer, &session).map_err(fixError::from)?;
        Ok(())
    }
...
mod tests {
    ...
    fn test_parse_layout() {
        ...
        let dms = WindowPath::Window(fixId::DirectList);
        let welcome = WindowPath::Window(fixId::Welcome);
    ...
    fn test_load_example_config_toml() {
        ...
        let config = fixConfig::load_toml(&path).expect("can load example_config.toml");
